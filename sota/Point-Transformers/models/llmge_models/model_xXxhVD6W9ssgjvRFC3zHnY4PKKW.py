...
class SA_Layer(nn.Module):
    ...
    def forward(self, x):
        q, k, v = self.split_and_process(x)
        attention = self.compute_attention(q, k)
        x_r = self.compute_output(v, attention)
        x_r = self.apply_norm_and_activation(x_r)
        return x + x_r

    def split_and_process(self, x):
    ...
    def compute_output(self, v, attention):
        x_r = torch.matmul(attention, v)
        x_r = self.trans_conv(x_r)
        return x_r

    def apply_norm_and_activation(self, x_r):
