...
class MixChannels(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([
            x1,
            self._mix_channels(x2),
            self._mix_channels(x3),
            self._mix_channels(x4)
        ], dim=1)
        return x

    def _mix_channels(self, x):
        g = self.channels // 4
        return ChannelShuffle(g)(x)
...
class StackedAttention(nn.Module):
    def __init__(self, channels=256):
        ...
        self.relu = nn.ReLU()
        self.mix_channels = MixChannels(channels)
