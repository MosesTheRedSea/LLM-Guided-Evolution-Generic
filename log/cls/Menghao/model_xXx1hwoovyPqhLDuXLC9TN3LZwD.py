...
class SA_Layer(nn.Module):
    def __init__(self, channels):
        ...
        self.after_norm = nn.BatchNorm1d(channels, eps=1e-5)
        self.act = nn.ReLU()
        ...
        nn.init.zeros_(self.q_conv.bias)
    ...
    def forward(self, x):
        ...
        # Scale dot product by sqrt(key_dimension)
        energy = torch.div(x_q @ x_k, torch.sqrt(torch.tensor(x_k.size(-1), device=x.device)))
        attention = self.softmax(energy)

        x_r = x_v @ attention
        x_r = self.after_norm(self.trans_conv(x - x_r))
        x_r = self.act(x_r)
