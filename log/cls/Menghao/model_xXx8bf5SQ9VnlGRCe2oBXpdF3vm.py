...
class StackedAttention(nn.Module):
    def __init__(self, channels=256, channel_last=True):
        super().__init__()
        self.channel_last = channel_last

        self.conv1 = nn.Conv2d(1, channels, kernel_size=(1, 1))
        self.conv2 = nn.Conv2d(channels, channels, kernel_size=(1, 1))

        self.bn = nn.BatchNorm2d(channels)

        # Placeholder implementation for SA_Layer
        class SA_Layer(nn.Module):
            def __init__(self, channels):
                super().__init__()
        ...
        if not self.channel_last:
            self.to_last = ChannelLast(nn.Identity())
            self.from_last = ChannelLast(nn.Identity())
